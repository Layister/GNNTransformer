...
def main():
    ...
    #dataset = LUNG(train=True, fold=fold, cell_feat_dim=cell_feat_dim)
    dataset = DLPFC(train=True, fold=fold, cell_feat_dim=cell_feat_dim)
    ...
    trainer.save_checkpoint(f"model/{dataname}_last_train_" + tag + '_' + str(fold) + ".ckpt")
